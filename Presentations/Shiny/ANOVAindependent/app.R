...
server <- function(input, output) {
  ...
  exampleSample <-eventReactive({input$do
    ...
    input$numsim}, {
      ...
      DV <- rnorm(input$samplen*input$numgroups,
                  input$normalmean,
                  input$normalsd)
      IV <- as.factor(rep(seq(1,input$numgroups,1),each=input$samplen))
   
      p_df<-data.frame(IV,DV)
      return(p_df)
